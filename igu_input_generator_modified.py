...
def fetch_igsdb_metadata(prod_id: int) -> dict:
    ...
    return {
        ...
        "coating_side": cs,
        "coating_name": cn,
        "emissivity_back": md.get("emissivity_back", 0.99)
    }
...
def get_effective_emissivity(meta: dict) -> float:
    """
    Returns the effective emissivity of a Low-E coated glass.
    Uses back emissivity by default, unless missing.
    """
    return meta.get("emissivity_back", 0.99)
...
def validate_surface_coating_conflicts(configs: list, igu_type: str) -> bool:
    """
    Validate that coatings are properly placed and don't conflict.
    Now allows the same coating on multiple glass lites (e.g., Glass 1 & 3 in triples).
    ...
    for i, config in enumerate(configs):
        ...
        # Determine which surfaces this glass occupies
        if igu_type == "Triple":
            surfaces = {
                0: [1, 2],  # Glass 1: surfaces 1 & 2
                1: [3, 4],  # Glass 2: surfaces 3 & 4 
                2: [5, 6]   # Glass 3: surfaces 5 & 6
            }
        else:  # Quad
            surfaces = {
                0: [1, 2],  # Glass 1: surfaces 1 & 2
                1: [3, 4],  # Glass 2: surfaces 3 & 4
                2: [5, 6],  # Glass 3: surfaces 5 & 6
                3: [7, 8]   # Glass 4: surfaces 7 & 8
            }
        
        glass_surfaces = surfaces.get(i, [])
        
        # Determine which surface the coating is on
        flip_needed = should_flip(
            ["outer", "quad_inner" if igu_type == "Quad" and i == 1 else "center", "center", "inner"][i],
            coating_side,
            coating_name
        )
        
        # After flipping, determine actual coating surface
        if flip_needed:
            coating_surface = glass_surfaces[0] if coating_side == "back" else glass_surfaces[1]
        else:
            coating_surface = glass_surfaces[1] if coating_side == "back" else glass_surfaces[0]
        
        # Check for conflicts on the same surface
        if coating_surface in surface_coatings:
            existing_coating = surface_coatings[coating_surface]
            # Only conflict if different coatings try to occupy same surface
            if existing_coating != coating_name:
                return False
        
        surface_coatings[coating_surface] = coating_name
            ...
            for o in outer_df.itertuples():
                for c in center_df.itertuples():
                    for i in inner_df.itertuples():
                        ...
                        # Validate surface coating conflicts (FIXED - now allows same coating on multiple lites)
                        glass_configs = [
                            {'meta': m_o, 'nfrc_id': o.NFRC_ID},
                            {'meta': m_c, 'nfrc_id': c.NFRC_ID},
                            {'meta': m_i, 'nfrc_id': i.NFRC_ID}
                        ]
                        if not validate_surface_coating_conflicts(glass_configs, "Triple"): continue
                        ...
                        # Check for Low-E dominance using effective emissivity
                        e_out = get_effective_emissivity(m_o)
                        e_in = get_effective_emissivity(m_i)
                        if e_in < e_out:
                            continue  # Reject if inner Low-E is stronger than outer (allow equal)
                ...
                for qi in center_df.itertuples():    # quad-inner candidate
                    ...
                    for c in center_df.itertuples():   # true-center candidate
                        ...
                        for i in inner_df.itertuples():
                            ...
                            if not validate_surface_coating_conflicts(glass_configs, "Quad"):
                                continue
                            ...
                            # Low-E dominance check
                            e_out = get_effective_emissivity(m_o)
                            e_in = get_effective_emissivity(m_i)
                            if e_in < e_out:
                                continue  # Inner Low-E stronger than outer (allow equal)
